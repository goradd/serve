...
const logModule = "http"

type bufferedOutputContext struct{}
...
func DisableOutputBuffering(ctx context.Context) {
	ctx.Value(bufferedOutputContext{}).(BufferedResponseWriterI).Disable()
}
...
func ResetOutputBuffer(ctx context.Context) []byte {
	...
	if i := ctx.Value(bufferedOutputContext{}); i == nil {
		return nil
...
func OutputLen(ctx context.Context) int {
	return ctx.Value(bufferedOutputContext{}).(BufferedResponseWriterI).Len()
}
...
// rather than a half finished output.
func WithBufferedOutput(next http.Handler) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		if bwriter.code != 0 && bwriter.code != 200 {
			...
			log.Error(ctx, logModule, "Application is attempting to send invalid header",
				slog.Any("header", bwriter.Header()))
		}
		_, e := w.Write(outBuf.Bytes())
		if e != nil {
			log.Error(ctx, logModule, "Buffered write error", slog.Any("error", e))
		}
	}
