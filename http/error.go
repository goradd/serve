...
// WithErrorHandler is middleware that inserts an error catcher into the handler stack that will
// catch downstream panics and prevent them from the stopping the server. They will instead be
// reported to the structured logger, and also be send a message to the browser in coordination
// with the goradd javascript.
//
