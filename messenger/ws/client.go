	...
	for {
		...
		if err != nil {
			if websocket.IsUnexpectedCloseError(err, websocket.CloseGoingAway) {
				// Could simply be that the browser was closed, or it could be a problem with our javascript
				log.Debug(nil, logModule, "Unexpected websocket close error", slog.Any("error", err))
			}
			break
		}
	...
	for {
		...
		case message, ok := <-c.send:
			...
			if buf, err2 := json.Marshal(messages); err2 != nil {
				...
				w.Write(buf)
				log.Debug(nil, logModule, "Write Pump", slog.String("value", string(buf)))
			}
...
func serveWs(hub *WebSocketHub, w http.ResponseWriter, r *http.Request, clientID string) {
	...
	if err != nil {
		...
		log.Error(nil, logModule, "Error upgrading connection. Make sure traffic is sent through a websocket protocol.",
			slog.Any("error", err),
			slog.String("request", string(dump)))
		return
	}
