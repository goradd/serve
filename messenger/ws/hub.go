...
func (h *WebSocketHub) run() {
	for {
		...
		case client := <-h.register:
			log.Debug(nil, logModule, "New client registering", slog.String("clientId", client.clientID))
		...
		case client := <-h.unregister:
			log.Debug(nil, logModule, "Client unregistering", slog.String("clientId", client.clientID))
			h.unregisterClient(client.clientID)

		case msg := <-h.send:
			if clientIDs, ok := h.channels[msg.Channel]; ok {
				log.Debug(nil, logModule, "Sending message", slog.Any("message", msg))
				...
				log.Debug(nil, logModule, "Could not find channel", slog.Any("message", msg))
			}

		case sub := <-h.subscribe:
			log.Debug(nil, logModule, "Subscribing to channel", slog.String("clientId", sub.clientID), slog.String("channel", sub.channel))
