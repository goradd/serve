...
type MessengerI interface {
	...
	// JavascriptFiles returns file names and attributes used to embed the needed javascript files for your messenger
	JavascriptFiles() map[string]html5tag.Attributes
}

// Send sends a message to the given channel via the current messenger
func Send(channel string, message any) {
	msg, err := json.Marshal(message)
	if err != nil {
		log.Warn(nil, "messenger", "JSON error marshalling message", slog.Any("error", err))
		return
	}
	if Messenger != nil {
